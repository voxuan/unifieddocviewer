...
public class ServiceSystemClient implements ExternalSystemClient {
    ...
    public List<ExternalDocumentDto> fetchDocuments(String vin) {
        ...
        if (config == null || !config.isEnabled()) {
            log.warn("ServiceSystemClient is disabled or not configured");
            return Collections.emptyList();
        }

        String baseUrl = config.getBaseUrl() != null ? config.getBaseUrl() : "http://localhost:8082";
        ...
        log.debug("Fetching service documents for VIN: {} from {}", vin, baseUrl);
